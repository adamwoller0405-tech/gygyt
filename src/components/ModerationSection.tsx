...
export const ModerationSection: React.FC<ModerationSectionProps> = ({
  ...
  const handleApproveRequest = async (req: JoinRequest) => {
    ...
    const newUser: UserProfile = {
      ...
      avatarUrl: DEFAULT_AVATAR, rank: UserRank.BRONZE,
      age: req.age, school: req.school,
      joinedDate: new Date().toISOString().split('T')[0],
      achievements: ['first_ride'],
      stats: { eventsJoined: 0 }
    };
    onUpdateUsers([...users, newUser]);
    toast(`${req.name} jóváhagyva!`);
  ...
  const handleToggleBan = (user: UserProfile) => {
    if (user.id === 'user_admin') { toast('A Rendszergazda nem tiltható ki!', 'error'); return; }
    setBanConfirmUser(user);
  };

  const handleSaveName = () => {
    if (!selectedUserToEdit || !editName.trim()) return;
    const updated = { ...selectedUserToEdit, name: editName.trim() };
    onUpdateUsers(users.map(u => u.id === selectedUserToEdit.id ? updated : u));
    setSelectedUserToEdit(updated);
    toast(`Név módosítva: ${editName.trim()}`);
  };

  const handlePasswordReset = async (email: string) => {
    if (!email) { toast('Nincs e-mail cím!', 'error'); return; }
    try {
      await sendPasswordResetEmail(auth, email);
      toast(`Jelszó-visszaállítási e-mail elküldve: ${email}`);
    } catch (err: any) { toast(`Hiba: ${err.message}`, 'error'); }
  };
  ...
  const nonAdminUsers = users.filter(u => u.rank !== UserRank.ADMIN);
  const totalNonAdmin = nonAdminUsers.length;
  const mutedCount = nonAdminUsers.filter(u => u.isMuted).length;
  const bannedCount = nonAdminUsers.filter(u => u.isBanned).length;
  ...
  return (
    <div className="flex flex-col h-full bg-bg-deep animate-fade-in overflow-hidden">
      <div className="px-5 py-4 bg-bg-panel border-b border-border-subtle sticky top-0 z-20 shadow-lg">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] font-black tracking-widest text-yellow-500 bg-yellow-500/10 px-3 py-1 rounded-full border border-yellow-500/20">
            BÉTA
          </span>
        </div>
        {/* Stat cards */}
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-black/40 border border-border-subtle rounded-2xl p-3 text-center">
            <p className="text-lg font-black text-brand-orange">{totalNonAdmin}</p>
            <p className="text-[8px] font-black uppercase text-neutral-500 tracking-wider">Tag</p>
          </div>
          <div className="bg-black/40 border border-border-subtle rounded-2xl p-3 text-center">
            <p className="text-lg font-black text-yellow-500">{pendingRequests.length}</p>
            <p className="text-[8px] font-black uppercase text-neutral-500 tracking-wider">Függő</p>
          </div>
          <div className="bg-black/40 border border-border-subtle rounded-2xl p-3 text-center">
            <p className="text-lg font-black text-red-500">{bannedCount}</p>
            <p className="text-[8px] font-black uppercase text-neutral-500 tracking-wider">Kitiltva</p>
          </div>
        </div>
        {mutedCount > 0 && (
          <div className="mt-2 text-center">
            <span className="text-[8px] font-black text-yellow-500/70">{mutedCount} némított tag</span>
          </div>
        )}
      </div>
      ...
      <div className="flex bg-black/40 mx-4 mt-3 p-1 rounded-2xl border border-border-subtle">
        {[
          ...
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            ...
            <t.icon size={14} />
            <span>{t.label}</span>
            <span className="text-[8px] opacity-70">({t.count})</span>
          </button>
        ))}
      </div>

      <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-24 max-w-lg mx-auto w-full">

        {activeTab === 'requests' && (
          <>
            {pendingRequests.length === 0 ? (
              <div className="bg-bg-card/50 border border-border-subtle rounded-[32px] p-10 text-center animate-fade-in">
                <UserCheck size={40} className="mx-auto text-neutral-800 mb-3" />
                <p className="text-xs text-neutral-600 font-bold uppercase tracking-widest">Nincsenek várakozó kérelmek</p>
              </div>
            ) : (
              pendingRequests.map(req => (
                <div key={req.id} className="bg-bg-card border border-border-card rounded-[32px] p-5 space-y-3 shadow-2xl animate-fade-in">
                  ...
                  <p className="text-[11px] text-neutral-400 italic bg-black/40 p-4 rounded-2xl border border-border-subtle leading-relaxed">"{req.introduction}"</p>
                  <div className="flex gap-3 pt-1">
                    <button onClick={() => handleRejectRequest(req)} disabled={loadingReqs.has(req.id)}
                      className="flex-1 py-3 rounded-2xl border border-red-500/20 bg-red-500/5 text-red-500 font-black text-[10px] uppercase tracking-widest hover:bg-red-500/10 transition-all active:scale-95 disabled:opacity-50">
                      {loadingReqs.has(req.id) ? <Loader2 size={14} className="animate-spin mx-auto" /> : 'Elutasít'}
                    </button>
                    <button onClick={() => handleApproveRequest(req)} disabled={loadingReqs.has(req.id)}
                      className="flex-1 py-3 rounded-2xl border border-green-500/20 bg-green-500/5 text-green-400 font-black text-[10px] uppercase tracking-widest hover:bg-green-500/10 transition-all active:scale-95 shadow-lg disabled:opacity-50">
            ...
            <div className="bg-bg-card rounded-[32px] border border-border-card divide-y divide-border-subtle/30 overflow-hidden shadow-2xl">
              {filteredUsers.length === 0 ? (
                <div className="p-8 text-center">
                  <p className="text-xs text-neutral-600 font-bold">Nincs találat</p>
                </div>
              ) : (
                filteredUsers.map(u => (
                  <div key={u.id}
                    className={`p-4 flex items-center justify-between cursor-pointer transition-colors ${selectedUserToEdit?.id === u.id ? 'bg-white/5' : 'hover:bg-white/5'}`}
                    onClick={() => { setSelectedUserToEdit(u); setEditName(u.name); }}>
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-2 mt-0.5">
                          ...
                          {u.isBanned && <span className="text-[8px] font-black bg-red-500/10 text-red-500 px-1 rounded">TILTVA</span>}
                        </div>
                      </div>
                    </div>
                    <div className="p-2 bg-black border border-border-subtle rounded-xl text-brand-orange hover:border-brand-orange/50 transition-colors">
      ...
      {selectedUserToEdit && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-5 backdrop-blur-2xl animate-fade-in">
          <div className="bg-bg-panel border border-border-subtle rounded-[48px] p-8 max-w-sm w-full space-y-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-border-subtle pb-5">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h3 className="text-sm font-black text-white uppercase">{selectedUserToEdit.name}</h3>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-[3px] text-neutral-500 ml-1">Név Módosítása</label>
              <div className="flex items-center space-x-2">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.values(UserRank).map(rk => (
                  <button key={rk} onClick={() => handleChangeRank(selectedUserToEdit.id, rk)}
                    className={`p-2.5 rounded-2xl border text-[9px] font-black uppercase transition-all ${selectedUserToEdit.rank === rk ? 'bg-brand-orange text-black border-brand-orange shadow-lg' : 'bg-black text-neutral-600 border-border-subtle hover:text-neutral-400'}`}>{rk}</button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-2 max-h-36 overflow-y-auto scrollbar-thin pr-2">
                {ACHIEVEMENTS.map(ach => {
                  const hasIt = selectedUserToEdit.achievements.includes(ach.id);
                  return (
                    <button key={ach.id} onClick={() => {
                      const newAch = hasIt ? selectedUserToEdit.achievements.filter(id => id !== ach.id) : [...selectedUserToEdit.achievements, ach.id];
                      ...
                      <div className="flex items-center space-x-2"><Award size={14} /><span className="text-[9px] font-black uppercase">{ach.title}</span></div>
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="flex gap-3">
                <button onClick={() => handleToggleMute(selectedUserToEdit)}
                  className={`flex-1 p-4 rounded-3xl border flex items-center justify-center space-x-3 transition-all font-black text-[10px] uppercase tracking-widest ${selectedUserToEdit.isMuted ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/30 shadow-inner' : 'bg-black text-neutral-400 border-border-subtle'}`}>
                  {selectedUserToEdit.isMuted ? <Volume2 size={16} /> : <VolumeX size={16} />}
                  <span>{selectedUserToEdit.isMuted ? 'Hang' : 'Némítás'}</span>
                </button>
                <button onClick={() => handleToggleBan(selectedUserToEdit)}
                  className={`flex-1 p-4 rounded-3xl border flex items-center justify-center space-x-3 transition-all font-black text-[10px] uppercase tracking-widest ${selectedUserToEdit.isBanned ? 'bg-red-500/20 text-red-500 border-red-500/40 shadow-inner' : 'bg-black text-red-600 border-red-900/30'}`}>
                  <ShieldX size={16} /><span>{selectedUserToEdit.isBanned ? 'Oldás' : 'Kitiltás'}</span>
                </button>
                <button onClick={() => setKickConfirmUser(selectedUserToEdit)}
                  ...
                  <Trash2 size={16} /><span>Kick</span>
                </button>
              </div>
            </div>

            {/* Password Reset */}
            <button onClick={() => handlePasswordReset(selectedUserToEdit.email)}
              className="w-full p-4 rounded-3xl border border-blue-900/30 bg-black text-blue-500 flex items-center justify-center space-x-3 transition-all font-black text-[10px] uppercase tracking-widest hover:bg-blue-500/10">
              <Key size={16} /><span>Jelszó visszaállítás</span>
            </button>
      ...
      <ConfirmDialog open={banConfirmUser !== null}
        ...
        onConfirm={() => { if (banConfirmUser) { onUpdateUsers(users.map(u => u.id === banConfirmUser.id ? { ...u, isBanned: !u.isBanned } : u)); toast(banConfirmUser.isBanned ? `${banConfirmUser.name} feloldva!` : `${banConfirmUser.name} kitiltva!`); setSelectedUserToEdit(null); setBanConfirmUser(null); } }}
        onCancel={() => setBanConfirmUser(null)} />

      <ConfirmDialog open={kickConfirmUser !== null} title="Tag eltávolítása"
        message={kickConfirmUser ? `BIZTOSAN ELTÁVOLÍTOD ${kickConfirmUser.name} tagot? Ez nem visszavonható!` : ''}
